        ...
        Establish context = () =>
                                {
                                    ...
                                    container.Setup(c => c.Get(first_convention_type)).Returns(first_convention_mock.Object);
                                    first_convention_mock.Setup(c => c.CanResolve(container.Object, service_type)).Returns(false);
                                    ...
                                    container.Setup(c => c.Get(second_convention_type)).Returns(second_convention);
                                    second_convention.CanResolveProperty = true;
                                };

        Because of = () => manager.Initialize();

        It should_not_resolve_on_first_convention = () => first_convention_mock.Verify(c => c.Resolve(container.Object, service_type), Times.Never());
