...
namespace Dolittle.PropertyBags.Specs
{
    public static class ShouldExtensions
    {
        ...
        static void IsAnAccurateRepresentationOf<T>(Value<T> source, Value<T> other)
            where T : Value<T>
        {
            if (other == null)
            ...
            foreach (var field in t.BaseType.GetMethod("GetFields", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(source, Array.Empty<object>()) as IEnumerable<FieldInfo>)
            {
