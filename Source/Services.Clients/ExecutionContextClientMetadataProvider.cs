...
namespace Dolittle.Services.Clients
{
    ...
    public class ExecutionContextClientMetadataProvider : ICanProvideClientMetadata
    {
        ...
        public IEnumerable<Metadata.Entry> Provide()
        {
            ...
            _logger.Trace("Setting execution context on call - Application: '{application}' Microservice: '{microservice}' TenantId: {tenant} - CorrelationId: {correlationId}", current.Application, current.Microservice, current.Tenant, current.CorrelationId);
