    ...
    public class Container : ICanPerformBootStage<ContainerSettings>
    {
        ...
        public void Perform(ContainerSettings settings, IBootStageBuilder builder)
        {
            ...
            if( settings.ContainerType != null ) 
            {
                ...
                resultingBindings = Dolittle.DependencyInversion.Booting.Boot.Start(assemblies, typeFinder, scheduler, fileSystem, logger, settings.ContainerType, bindings);
            } 
            else 
            {
                var bootResult = Dolittle.DependencyInversion.Booting.Boot.Start(assemblies, typeFinder, scheduler, fileSystem, logger, bindings);
