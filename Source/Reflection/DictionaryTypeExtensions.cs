...
namespace Dolittle.Reflection
{
    ...
    {
        readonly static HashSet<Type> GenericDictionaryInterfaces = new HashSet<Type>
        {
            typeof(IDictionary<,>),
            typeof(IReadOnlyDictionary<,>)
        };
        readonly static HashSet<Type> NonGenericDictionaryInterfaces = new HashSet<Type>
        {
            typeof(IDictionary),
        };

        readonly static HashSet<Type> DictionaryInterfaces;
        ...
        {
            return type.GetInterfaces().Append(type).Where(t => DictionaryInterfaces.Any(i => i == t || t.GetTypeInfo().IsGenericType && i == t.GetGenericTypeDefinition())).FirstOrDefault();
        }
        ...
        public static bool IsReadOnlyDictionary(this Type type)
        {
            ...
            if (typeof(Dictionary<,>).MakeGenericType(keyType, valueType).IsAssignableFrom(type)) return false;
